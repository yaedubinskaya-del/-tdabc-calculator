...
from config import AssistantConfig, InsuranceRates, KpiPerProcedure, KpiRevenue, Procedure, WEEKS_PER_MONTH
...
def _parse_procedures_df(df) -> tuple[list[Procedure] | None, str | None]:
    """Парсит процедуры из Gradio Dataframe. Возвращает (список, ошибка)."""
    rows = []
    try:
        import pandas as pd
    ...
    for i, row in enumerate(rows, 1):
        ...
        if not name or name in ("", "nan", "None"):
            continue
        try:
            minutes = float(str(row[1]).replace(",", "."))
            volume = int(float(str(row[2]).replace(",", ".")))
        except (ValueError, TypeError):
            return None, f"Строка {i} ({name}): минуты и объём должны быть числами"
        if minutes <= 0:
            continue
        if volume < 0:
            return None, f"Строка {i} ({name}): объём не может быть отрицательным"
        procedures.append(Procedure(name=name, minutes=minutes, monthly_volume=volume))

    if not procedures:
        return None, "Заполните хотя бы одну процедуру с ненулевым временем"
...
def _calculate_from_form(
    ...
    procedures, err = _parse_procedures_df(procedures_df)
    if err:
...
def launch_web(share: bool = False) -> None:
    ...
    with gr.Blocks(
        ...
        schedule_mode = gr.Radio(
            choices=[
                ("Ввести непациентское время вручную", "manual"),
                ("Рассчитать автоматически из процедур", "auto"),
            ],
            value="auto",
            label="Режим расчёта непациентского времени",
            info="'Авто' — вводите общее кол-во рабочих часов в месяц, непациентское время считается само",
        )
        with gr.Row():
            total_hours_month = gr.Number(
                label="Рабочих часов в месяц (для авторасчёта)",
                value=160,
                info="Например: 160 ч (5 дн × 8 ч × 4 нед) или фактическое число из табеля",
                visible=True,
            )
            non_patient_min_per_day = gr.Number(
                label="Непациентское время, мин/день (вручную)",
                value=45,
                info="Обед + уборка + подготовка + документация",
                visible=False,
            )

        def toggle_schedule(mode):
            return (
                gr.update(visible=(mode == "auto")),
        ...
        schedule_mode.change(
            ...
            outputs=[total_hours_month, non_patient_min_per_day],
        )
        ...
        gr.Markdown(
            "Заполните таблицу: название процедуры, время участия сотрудника (минуты), "
            "количество процедур в месяц. Строки можно добавлять."
        )
        procedures_df = gr.Dataframe(
            headers=["Процедура", "Минуты", "Объём/мес"],
            datatype=["str", "number", "number"],
            value=[
                ["Гистероскопия", 20, 0],
                ["Большая операция", 60, 0],
                ["Пункция фолликулов", 30, 0],
                ["", 0, 0],
                ["", 0, 0],
            ],
        ...
        def run_calc(
            ...
            sched_mode, non_patient_min, total_hours,
        ...
        calculate_btn.click(
            ...
            inputs=[
                name, role, salary, insurance_rate,
                days_per_week, hours_per_day,
                schedule_mode, non_patient_min_per_day, total_hours_month,
